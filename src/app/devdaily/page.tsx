...
import dailyData from "./components/daily";
            ...
            <div className="flex h-[80vh] w-full flex-col gap-3">
                ...
                <div className="h-full w-full md:w-[75%] flex flex-col gap-3 mx-auto overflow-y-auto">
                    { 
                        dailys.map(daily => {
                            return (
                                <div key={daily.title} className={`p-3 border border-gray-300 rounded shadow flex flex-col gap-3 ${bgColor(daily.topic)} hover:shadow-lg transition-all duration-500`}>
                                    <div>
                                        <h4 className="font-bold">{dailyEmoji(daily.topic)} {daily.title}</h4>
                                        <Tag variant="solid" color={tagColor(daily.topic)}>{daily.topic}</Tag>
                                        <span> </span>
                                        <Tag variant="solid" color="#888888">{daily.date}</Tag>
                                    </div>
                                    <div>
                                        {daily.description}
                                    </div>
